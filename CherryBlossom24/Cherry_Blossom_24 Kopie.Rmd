...
To predict the bloom dates of the years 2025 - 2034, I will use predicted DRD dates, temperatures as well as the predicted temperature in March and April (macht das Sinn? Oder soll ich hier einfach direkt nur die früheren Jahre als Prädiktor nehmen?)
...
text(DRD23 +1,  220, "Dormacy Release Date", cex = 0.6, col = "red", pos = 4)
...
DRD. However, the ghcnd weather data ends in fall 2022, so I looked for more current temperature data online and retrieved free temperature data from https://www.visualcrossing.com. These data sets include the weather data between 10/01/2023 and 02/29/2024 (forecast) for each city. <br>
...
rmse(dat_Wash$bloom_doy, pbloomW) # 5.07
mae(dat_Wash$bloom_doy, pbloomW) # 4.19
cor(dat_Wash$bloom_doy, pbloomW) # 0.73
...
<span style="color: red;">My prediction for Washington is May 26th 2024</span>
...
rmse(dat_lies$bloom_doy, pbloomL2) # 7.11
mae(dat_lies$bloom_doy, pbloomL2) # 5.32
cor(dat_lies$bloom_doy, pbloomL2) # 0.81
...
The predictions are on average wrong by 8 days which isn't very good. However,  I tried a few different things and nothing seemed to work better, so I will go with it. <br>
...
mAllV <- lm(bloom_doy ~ year + DRD * t.pred + mean.jan, dat_Vanc)
...
rmse(dat_Vanc$bloom_doy, pbloomV) # 4.17
mae(dat_Vanc$bloom_doy, pbloomV) # 2.77
cor(dat_Vanc$bloom_doy, pbloomV) # 0.79
...
### Different Models
mYearN <- lm(bloom_doy ~ year, dat_NY)
# summary(mYearV) # R^2 = 1
# This model has a model fit of 1, probably because I predicted the past bloom 
# dates using year. I will thus not use year as predictor. 

mDRDN <- lm(bloom_doy ~ DRD, dat_NY)
# summary(mDRDV) # R^2 = 0.11
mtpredN <- lm(bloom_doy ~ t.pred, dat_NY)
# summary(mtpredV) # R^2 = 0.13
mjanN <- lm(bloom_doy ~ mean.jan, dat_NY)
# summary(mjanV) # R^2 = 0.27
mAllN <- lm(bloom_doy ~ DRD * t.pred + mean.jan, dat_NY)
summary(mAllV) # R^2 = 0.46
# I will use the model with everything but year as predictors. 

# check prediction accuracy: 
pbloomN <- predict(mAllN, newdata = data.frame(year = dat_NY$year, 
  DRD = dat_NY$DRD, t.pred = dat_NY$t.pred, mean.jan = dat_NY$t.pred))
rmse(dat_NY$bloom_doy, pbloomN) # 2.55
mae(dat_NY$bloom_doy, pbloomN) # 2.21
cor(dat_NY$bloom_doy, pbloomN) # 0.54
# of course, this needs to be interpreted with caution, since the DV (past blooom
# dates) are not the real ones. 
